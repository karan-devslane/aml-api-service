...
class SkillService {
  ...
  async fetchSkillIdsByName(): Promise<Map<string, { id: number; name: string; type: string }>> {
  ...
  async getSkillByIdAndType(skillId: string, type: SkillType) {
  ...
  async checkSkillsExistByIds(skillIds: number[]): Promise<boolean> {
    const skills = await SkillMaster.findAll({
      where: {
        identifier: skillIds,
  ...
  // Update skill
  async updateSkillData(skillId: string, req: any): Promise<any> {
  ...
  async fetchSkillsByIds(skillIds: string[]): Promise<SkillMaster[]> {
  ...
  async getSkillList(req: Record<string, any>) {
